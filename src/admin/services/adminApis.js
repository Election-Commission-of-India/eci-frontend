...
export const assignRole = async (userId, role) => {
  const response = await apiClient.put(`/api/admin/users/${userId}/assign-role`, null, {
    params: { role }
  });
  return response.data;
};
...
export const createConstituency = async (constituencyData) => {
  const response = await apiClient.post('/api/admin/constituencies', constituencyData);
  return response.data;
};

export const updateConstituency = async (id, constituencyData) => {
  const response = await apiClient.put(`/api/admin/constituencies/${id}`, constituencyData);
  return response.data;
};

export const toggleConstituencyStatus = async (id) => {
  const response = await apiClient.put(`/api/admin/constituencies/${id}/toggle-status`);
  return response.data;
};

// Polling Station Management APIs
export const createPollingStation = async (stationData) => {
  const response = await apiClient.post('/api/admin/polling-stations', stationData);
  return response.data;
};

export const updatePollingStation = async (id, stationData) => {
  const response = await apiClient.put(`/api/admin/polling-stations/${id}`, stationData);
  return response.data;
};

export const assignPollingStationToConstituency = async (stationId, constituencyId) => {
  const response = await apiClient.put(`/api/admin/polling-stations/${stationId}/assign-constituency/${constituencyId}`);
  return response.data;
};

export const checkPollingStationAssignment = async (stationId) => {
  const response = await apiClient.get(`/api/admin/polling-stations/${stationId}/assignment`);
  return response.data;
};

// Document Type Management APIs
export const createDocumentType = async (documentTypeData) => {
  const response = await apiClient.post('/api/admin/document-types', documentTypeData);
  return response.data;
};
