...
export default function AssignedApplications() {
  ...
  // Based on backend ApplicationStatus enum
  ...
  const fetchAssignedApplications = async () => {
    try {
      ...
      const data = await getAssignedApplications(userId);
  ...
  const applyFilters = async () => {
    try {
      ...
      const data = await filterApplications(userId, cleanFilters);
      setApplications(data);
    } catch (error) {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const data = await searchApplications(userId, searchTerm);
      setApplications(data);
    } catch (error) {
  ...
  const viewApplicationDetails = (applicationId) => {
    navigate(`/blo/applications/${applicationId}/details`);
  };
          ...
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 mt-1">View and manage applications assigned to you</p>
              </div>
            </div>
            <button
